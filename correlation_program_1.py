...
V = [0, 1, 2, 3, 4, 5]
T = [0, 1, 2, 3, 4, 5]
N = [ [100, 100, 200, 200, 300, 200], [0, 10, 50, 100, 190, 200], [0, 0, 0, 20, 30, 50], [60, 50, 30, 30, 20, 20], [100, 200, 100, 200, 200, 100], [100, 100, 100, 200, 250, 300]]
...
for t in T:
    XA[t] = m.addVar(vtype = GRB.INTEGER, name = "XA["+str(t)+"]")
    XB[t] = m.addVar(vtype = GRB.INTEGER, name = "XB["+str(t)+"]")
...
SA = m.addVar(vtype = GRB.INTEGER, name = "SA")
SB = m.addVar(vtype = GRB.INTEGER, name = "SB")
...
m.addConstr( SA - 1 <= quicksum(Z2A[l] for l in L), name = "RC15")
m.addConstr( SA >= quicksum(Z2A[l] for l in L), name = "RC16")
m.addConstr( SB - 1 <= quicksum(Z2B[l] for l in L), name = "RC17")
m.addConstr( SB >= quicksum(Z2B[l] for l in L), name = "RC18")
...
for l in L:
    for ll in L:
        ...
        m.addConstr( Z2[l, ll] <= Z2B[l], name = "RC20["+str(l)+","+str(ll)+"]")
